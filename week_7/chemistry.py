def main():
    ...
    name = check_name(formula)
    parse_list = parse_formula(formula, elements)
    mass_tot = compute_molar_mass(parse_list, elements)
    moles = mole_calculator(mass_tot, mass_in_grams)
    
    print (f"Name: {name}")
    print (f"{mass_tot:.5f} grams/mole")
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for item in symbol_quantity_list:
        ...
        if name in periodic_table_dict:
            element = periodic_table_dict[name]
...
def mole_calculator(mass_tot, sample_mass):
    moles = sample_mass/mass_tot
    return moles

def check_name(molecule):
    ...
    if molecule in known_molecules_dict:
        knownMolecule = known_molecules_dict[molecule]
        name = knownMolecule
    else:
